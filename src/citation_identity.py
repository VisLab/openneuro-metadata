"""
citation_identity.py — Synced copy of task-research's identity module.

UPSTREAM
    task-research:Claude-research/code/literature_search/identity.py

    This file is a verbatim functional copy.  Do not edit it locally.
    Make changes upstream and re-sync the file here.  Drift between
    the two copies is detected by tests/test_citation_identity.py,
    which pins three known (family, year, title) -> pub_id triples;
    those tests fail loudly if either copy of the module diverges.

WHY THIS FILE EXISTS
    The OpenNeuro citation registry (datasets/dataset_summaries/
    citation_registry.tsv, post-Phase-2 redesign) carries a `pub_id`
    column that bridges to task-research's publications catalog.  For
    that bridge to work, both repos must compute identical pub_ids
    from identical (family, year, title) inputs.  The cleanest way to
    guarantee that is to share the implementation.  Until a packaged
    upstream exists, "shared implementation" means "copy with a
    sync-comment and pinned-hash drift detection."

    See openneuro-metadata:.status/citation_id_design_v2.md §6 and
    .status/cross_repo_id_thinking_2026-05-01.md §6 for the design.

PURPOSE (from upstream module docstring)
    Deterministic pub_id, canonical_string, and PDF filename
    generation.  Pure functions: same inputs always produce the same
    outputs.  No network calls, no I/O, no global state.

    Algorithm source: task-research's
    instructions/literature_search_plan_2026-04-21.md §11.7
    (authoritative) and pdf_naming_thinking_2026-04-21.md §5.

      build_canonical_string(first_author_family, year, title) -> str
          Returns the <=100-char lowercase alphanumeric string that is
          the SHA-1 input for both pub_id and the PDF filename's
          trailing hash.
...
TODO (from upstream): when writing a new pub_id into publications.json,
check for 8-char hash collisions and bump to 10 chars for any colliding
pair.  This is a future Phase 6 concern in task-research; OpenNeuro's
resolver inherits whichever decision is made upstream.
"""
